...
export const userLoginService = async ({
  ...
  const match = await comparePassword({
    password,
    existingPassword: findUserByEmail.password,
  });

  if (!match) throw { msg: "Password anda salah", status: 400 };

  const setToken = tokenSign({
    id: findUserByEmail.id,
    role: findUserByEmail.role,
  });

  const setRefreshToken = refrestTokenSign({
    id: findUserByEmail.id,
    role: findUserByEmail.role,
  });
...
export const userSetPasswordService = async ({
  ...
  const findUserById = await prisma.user.findFirst({
    where: {
      AND: [{ id: userId }, { isUpdatePassword: false }],
    },
  });

  if (findUserById?.tokenUpdatePassword !== tokenRequest)
    throw {
      msg: "Link sudah tidak berlaku, silahkan kirim ulang permintaan",
      status: 400,
    };

  if (!findUserById)
    throw {
      msg: "Permintaan ubah password sudah digunakan. Silakan kirim ulang permintaan untuk mengganti password lagi.",
      status: 400,
    };
...
//   const readFileHtml = readFileSync(
//     "./src/public/emailHtml/forgotPassword.html",
//     "utf-8"
//   );
//   let compiledHtml: any = compile(readFileHtml);
//   compiledHtml = compiledHtml({
