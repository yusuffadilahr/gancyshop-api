...
export const userRegister = async (
  ...
  try {
    ...
    if (!checkCharacter(firstName) || !checkCharacter(lastName))
      throw {
        msg: "Nama tidak boleh mengandung karakter atau angka",
        status: 400,
      };

    if (!checkNumberInput(phoneNumber))
...
export const userLogin = async (
  ...
  try {
    ...
    const { token, findUserByEmail, refreshToken } = await userLoginService({
      email,
      password,
    });
...
export const userSetPassword = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const { response } = await userSetPasswordService({
      password,
      tokenRequest: tokenRequest as string,
...
export const userForgotPassword = async (
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {},
      message:
        "Berhasil mengirim ulang, harap melakukan pengecekan email anda secara berkala",
    });
  } catch (error) {
    next(error);
  }
...
export const userChatByUserId = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
